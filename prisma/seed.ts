...
async function main() {
  // Check if a contract template already exists
  const existingTemplate = await db.contractTemplate.findFirst();
  ...
  if (!existingTemplate) {
    ...
    await db.contractTemplate.create({
      data: {
        ...
        content: `
          ...
          1. Contribute the specified amount on the agreed schedule.
          2. Adhere to the payout order and terms set by the group.
          3. Abide by the rules of the group, including legal action in case of non-compliance.
    ...
    console.log('Default contract template seeded successfully.');
  } else {
    console.log('Contract template already exists. Skipping seeding.');
  }
}
