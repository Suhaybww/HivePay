...
// Removed: determineTicketPriority, getAIResponse
// (We keep the same references to sendTicketEmail, notifySupportTeam.)
...
export const supportRouter = router({
  ...
  createTicket: privateProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      try {
        ...
        const priority: TicketPriority = manualPriority
          ? (manualPriority as TicketPriority)
          : "Medium";
        ...
        // Send email notification to the user (no AI response now)
        ...
        await notifySupportTeam(ticket.id, dbUser, subject, priority, message);
  ...
  getTicketStats: privateProcedure.query(async ({ ctx }) => {
    const { user } = ctx;
      ...
      try {
        ...
        const response = await db.ticketResponse.create({
          data: {
            ...
            staffEmail,
            ticketId,
            // Not including userId because it's staff
          },
        });
