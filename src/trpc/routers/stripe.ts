...
export const stripeRouter = router({
  ...
  .mutation(async ({ ctx }) => {
    ...
    try {
      // Create new SetupIntent whether setting up or updating
...
// New method to handle webhook events for BECS setup
...
// New method to get BECS setup status
getBECSStatus: privateProcedure
.query(async ({ ctx }) => {
  const { userId } = ctx;
